...
public class AnsattDAO {
	...
	public Ansatt finnAnsattMedBrukernavn(String brukernavn) {
		...
		Ansatt ansatt = null;
		try {
			ansatt = em.createQuery("SELECT a FROM Ansatt a WHERE a.brukernavn = :brukernavn", Ansatt.class)
	...
	public void leggTilAnsatt(String brukernavn, String fornavn, String etternavn, String dato, String stilling,
			int lønn, int avdelingId) {
		EntityManager em = emf.createEntityManager();
		EntityTransaction tx = em.getTransaction();
	...
	public void fjernAnsatt(int ansattId) {
		EntityManager em = emf.createEntityManager();
		try {
			em.getTransaction().begin();

			// Hent ansatt fra databasen
			Ansatt ansatt = em.find(Ansatt.class, ansattId);

			if (ansatt != null) {
				em.remove(ansatt); // Fjern ansatt fra databasen
				em.getTransaction().commit();
				System.out.println("Ansatt med ID " + ansattId + " er fjernet.");
			} else {
				System.out.println("Ingen ansatt funnet med ID " + ansattId);
			}
		} catch (Exception e) {
			em.getTransaction().rollback();
			System.err.println("Feil ved sletting av ansatt: " + e.getMessage());
		} finally {
			em.close();
		}
	}
